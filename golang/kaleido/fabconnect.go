...
func NewFabconnectClient(fabconnectUrl, username string) (*FabconnectClient, error) {
	...
	// FIX: Use FABCONNECT_URL to build WebSocket URL instead of hardcoded localhost
	wsUrl := os.Getenv("FABCONNECT_WS_URL")
	if wsUrl == "" {
		// Convert HTTP(S) URL to WebSocket URL
		wsUrl = strings.Replace(fabconnectUrl, "https://", "wss://", 1)
	...
	if err != nil {
		log.Errorf("Failed to connect to websocket at %s. %v", wsUrl, err)
		return nil, err
	}
